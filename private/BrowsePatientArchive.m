...
% BrowsePatientArchive is called by TomoDoseRate when the user clicks the
...
if ~isequal(handles.name, 0)
    ...
    if isletter(handles.db(1))
        ...
        set(handles.plan_menu, 'String', plans(:,2));
        ...
        if length(handles.planUIDs) >= 1
            ...
            set(handles.plan_menu, 'enable', 'on');
            
            % Execute plan_select
            handles = SelectPlan(handles, ...
                get(handles.plan_menu, 'Value'));
    ...
    elseif str2double(handles.db(1)) >= 2
        ...
        % Update plan dropdown menu
        set(handles.plan_menu, 'String', handles.planUIDs);
